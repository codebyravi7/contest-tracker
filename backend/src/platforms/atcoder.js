...
const parseTable = ($, tbody) => {
  ...
  tbody.find("tr").each((index, element) => {
    ...
    const startTimeIso = startTimeHref
      ? startTimeHref.split("=")[1].split("&")[0]
      : "";
    const formattedStartTimeIso = `${startTimeIso.substring(
      0,
      4
    )}-${startTimeIso.substring(4, 6)}-${startTimeIso.substring(
      6,
      8
    )}T${startTimeIso.substring(9, 11)}:${startTimeIso.substring(11)}`;
    ...
    const startTime = new Date(
      startTimeJST.getTime() - 9 * 60 * 60 * 1000
    ).getTime(); // To UTC
